...
const CASHFLOW_DRAGABLE_ELEMENT = document.querySelector(
  "#monthlyCashflowInnerContainer"
);
...
CASHFLOW_DRAGABLE_ELEMENT.style.left =
  "calc(" + leftPropertyScreen + "vw + " + leftPropertyDelta + "px)";
...
CASHFLOW_DRAGABLE_ELEMENT.addEventListener(
  "touchmove",
  function(e) {
    ...
    if (Math.abs(touchDelta) <= 100) {
      ...
      CASHFLOW_DRAGABLE_ELEMENT.style.left =
        "calc(" + leftPropertyScreen + "vw + " + leftPropertyTouchDelta + "px)";
    }
  },
  false
);
...
var exchangeRequest = new XMLHttpRequest();
console.log('BED');
...
window.addEventListener("load", function counter() {
  ...
  const DEPOSITS_PARSE = JSON.parse(sessionStorage.getItem("deposits"));
  const CRYPTO_PARSE = JSON.parse(sessionStorage.getItem("crypto"));
  ...
  function setStartDate(i) {
    return Date.parse(DEPOSITS_PARSE[i].startDate);
  }

  function setStartAmount(i) {
    return parseFloat(DEPOSITS_PARSE[i].startAmount);
  }

  function setInterestRate(i) {
    return parseFloat(DEPOSITS_PARSE[i].interestRate);
  }

  if (
    !sessionStorage.getItem("deposits") ||
    sessionStorage.getItem("deposits") !== JSON.stringify(deposits)
  ) {
    location.reload();
    sessionStorage.setItem("deposits", JSON.stringify(deposits));
  }

  if (
    !sessionStorage.getItem("crypto") ||
    sessionStorage.getItem("crypto") !== JSON.stringify(crypto)
  ) {
    location.reload();
    sessionStorage.setItem("crypto", JSON.stringify(crypto));
  }

  const nextGoal = parseInt(localStorage.getItem("nextGoal"));

  for (var i = 0; i < DEPOSITS_PARSE.length; i++) {
